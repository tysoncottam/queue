    ...
    <AppShell user={session.user}>
      ...
      {rows.length === 0 ? (
        <div className="mt-12 text-center text-sm text-muted">
          ...
          <Link
            href="/searches/new"
            className="mt-3 inline-block text-foreground underline"
          >
                ...
                <dl className="space-y-1.5 text-xs">
                  <Row label="Videos">
                    {keywords ? (
                      <>
                        ...
                        {keywords.split(/[,\n]/).map((k, i, arr) => (
                          <span key={i}>
                            <span className="rounded-sm bg-surface-raised px-1 py-0.5 font-medium text-foreground">
                              {k.trim()}
                            </span>
                            {i < arr.length - 1 ? " or " : ""}
                          </span>
                  ...
                  <Row label="Polling">
                    {s.active ? (
                      ...
                      <span className="text-muted">Paused — not checking</span>
                    )}
                  </Row>
                </dl>
...
function formatDate(iso: string): string {
  // Parse YYYY-MM-DD as a local date (avoiding timezone drift from Date(string))
